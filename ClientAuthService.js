...
function getClientPosts(clientId, authorizedClient) {
  try {
    ...
    // Build a set of post IDs that have ever been submitted for client review.
    // This lets clients continue to see posts that went back to Internal_Review
    // after they requested changes.
    var clientApprovals = _readSheetAsObjects_('Post_Approvals', {
      filterFn: function(a) {
        return a.Approval_Stage === 'Client' || a.Approval_Stage === 'Client_Review';
      }
    });
    var everSubmittedToClient = {};
    for (var i = 0; i < clientApprovals.length; i++) {
      everSubmittedToClient[clientApprovals[i].Post_ID] = true;
    }

    // Show posts that are in a client-visible status OR that were previously
    // submitted for client review (so clients never lose progress tracking).
    ...
    var posts = _readSheetAsObjects_('Posts', {
      filterFn: function(p) {
        ...
        return clientVisibleStatuses[p.Status] || everSubmittedToClient[p.ID];
      },
      coerceFn: function(p) {
